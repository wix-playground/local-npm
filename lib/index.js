...
module.exports = function (argv) {
  ...
  app.get('/tarballs/:name/:version.tgz', function (req, res) {
    ...
    var pkgId = pkgName + '-' + pkgVersion;

    db.get(pkgId, {asBuffer: true, valueEncoding: 'binary'}, function (err, resp) {
      if (!err) {
      ...
      var get = request.get('https://registry.npmjs.org/' + pkgName +
      ...
      get.pipe(through(function (chunk, _, next) {
        ...
        if (dist.shasum === hash.digest('hex')) {
          db.put(pkgId, buff, function (err) {
